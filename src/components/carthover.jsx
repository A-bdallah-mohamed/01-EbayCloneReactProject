             ...
             <div className='mt-[20px] overflow-y-auto h-auto max-h-[300px] mb-8 gap-[20px] flex flex-col'>
               {Uniquecart.map((product , id) => (
                <div key={id} className='min-h-[100px] max-h-[100px] flex flex-col relative'>
                    <div className='flex'>
<div className='min-w-[90px] max-w-[90px] min-h-full max-h-full flex items-center justify-center bg-slate-200'>   <Link to={`/pages/Productpage/${product.id}`}><img src={product.thumbnail} className='cursor-pointer max-h-[100px]' /></Link></div>
<div className='flex flex-col'>
<Link to={`/pages/Productpage/${product.id}`}><div className='text-[15px] px-2'>{product.title}</div></Link>
